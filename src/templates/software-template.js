...
export const softwareQuery = graphql`
  query($slug: String!) {
    md: markdownRemark(frontmatter: {cat: {eq: "software"}, path: {eq: $slug}}) {
      frontmatter {
        title
        path
        parentPath
    ...
    subMenuLevel2: allMarkdownRemark(sort: {fields: frontmatter___id}, filter: {frontmatter: {cat: {eq: "software"}}}) {
      edges {
        node {
          frontmatter {
            title
    ...
    content: markdownRemark(frontmatter: {cat: {eq: "software"}, path: {eq: $slug}}) {
      frontmatter {
        title
        path
        parentPath
