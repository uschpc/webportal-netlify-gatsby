      ...
      <Layout {...data.navigation}>
          ...
          <div className="nav-pages">
            <div className="container">
                ...
                <h1>{data.researcher.frontmatter.title}</h1>
                <Pagination {...data.allResearcher} isSafari={isSafari} />
                </div>
...
export const allResearcherQuery = graphql`
  query {
    ...
    allResearcher: allMarkdownRemark(filter: {frontmatter: {cat: {eq: "Researchers"}}}, sort: {fields: frontmatter___date, order: DESC}) {
      edges {
        node {
          frontmatter {
            title
    ...
    researcher: markdownRemark(frontmatter: {cat: {eq: "allResearcher"}}) {
      frontmatter {
        title
        parentEle
        cat
