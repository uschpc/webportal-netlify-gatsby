...
export default function Template({ data }) {
  let subMenu = findSubMenu(data.project.frontmatter.parentEle, data.sideMenu)
          ...
          <div className="nav-pages">
            <div className="container">
                <div className="left-column">
                  <h3>{data.project.frontmatter.parentEle}</h3>
                  {subMenu.map((item, i) => {
                  return (
...
export const allProjectsQuery = graphql`
  query {
    ...
    allProjects: allMarkdownRemark(filter: {frontmatter: {cat: {eq: "projects"}}}, sort: {fields: frontmatter___date, order: DESC}) {
      edges {
        node {
          frontmatter {
            title
    ...
    project: markdownRemark(frontmatter: {cat: {eq: "allProjects"}}) {
      frontmatter {
        title
        parentEle
        cat
